...
use crate::ui::styles::{block_style, list_style};
use ratatui::Frame;
...
pub fn render(frame: &mut Frame, area: Rect, state: &mut AppState) {
    ...
    let selected = state.config_editor.list_state.selected();
    ...
    let items = state
        ...
        .map(|(idx, item)| {
            ...
            ListItem::new(text).style(list_style(selected.is_none_or(|s| s == idx)))
        });
