...
jira:
    ...
    #[test]
    fn read_config_returns_err_when_unable_to_read_file_content() {
        let dir = TempDir::new().unwrap();
        let file_path = temp_loader_path(&dir);
        let config_loader = ConfigLoader::from_path(file_path);
        assert!(matches!(config_loader.read_config(), Err(ConfigError::Read(_))));
    }

    #[test]
    fn read_config_returns_err_when_unable_to_decode_yaml() {
        let dir = TempDir::new().unwrap();
        let file_path = temp_loader_path(&dir);
        let json = "{}";
        fs::write(&file_path, json).expect("Unable to write to temp file");
        let config_loader = ConfigLoader::from_path(file_path);
        assert!(matches!(config_loader.read_config(), Err(ConfigError::Parse(_))));
    }
